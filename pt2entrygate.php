...
//Patient entry page for day 2.  This page will log the patient into the entry page             //
...
if (!isset($_SESSION['username']))
{
    if (isset($_GET['p']))
    {
        ...
        $day1code = $_GET['p'];
        $getPIDQ = "SELECT patientsid FROM codes WHERE day2code LIKE '".$day2code."' AND day2used = 0";
        ...
        if (mysqli_num_rows($getPIDA) == 1)
        {
            ...
            if (mysqli_num_rows($getLIA) == 1)
            {
                ...
                $setUQ = "UPDATE codes SET day2used = 1 WHERE patientsid = ".$_SESSION['currentptnum'];
...
{
    $setUQ = "UPDATE codes AS a INNER JOIN patientusers AS b ON a.patientsid=b.patientsid SET a.day2used = 1 WHERE b.username = '".$_SESSION['username']."'";
    mysqli_query($dbc, $setUQ);
    echo'
    <script>
    window.location = "pt2entry.php";
