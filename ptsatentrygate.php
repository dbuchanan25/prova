...
if (!isset($_SESSION['username']))
{
    if (isset($_GET['p']))
    {
        ...
        $ptsatcode = $_GET['p'];
        $getPIDQ = "SELECT patientsid FROM codes WHERE ptsatcode LIKE '".$ptsatcode."' AND ptsatused = 0";
        ...
        if (mysqli_num_rows($getPIDA) == 1)
        {
            ...
            $getLIQ = "SELECT a.username, b.phone FROM patientusers AS a INNER JOIN patients AS b ON a.patientsid=b.id "
                    . "WHERE a.patientsid=".$_SESSION['currentptnum'];
            ...
            if (mysqli_num_rows($getLIA) == 1)
            {
                ...
                $setUQ = "UPDATE codes SET ptsatused = 1 WHERE patientsid = ".$_SESSION['currentptnum'];
                mysqli_query($dbc, $setUQ);
                echo'
                <script>
                window.location = "ptsatentry.php";
                </script>';
            }
            else
            {
...
{
    $setUQ = "UPDATE codes AS a INNER JOIN patientusers AS b ON a.patientsid=b.patientsid SET a.ptsatused = 1 WHERE b.username = '".$_SESSION['username']."'";
    mysqli_query($dbc, $setUQ);
    echo'
    <script>
    window.location = "ptsatentry.php";
