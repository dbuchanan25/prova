...
//Patient entry page for day 4.  This page will log the patient into the entry page             //
...
if (!isset($_SESSION['username']))
{
    if (isset($_GET['p']))
    {
        ...
        $day1code = $_GET['p'];
        $getPIDQ = "SELECT patientsid FROM codes WHERE day4code LIKE '".$day4code."' AND day4used = 0";
        ...
        if (mysqli_num_rows($getPIDA) == 1)
        {
            ...
            if (mysqli_num_rows($getLIA) == 1)
            {
                ...
                $setUQ = "UPDATE codes SET day4used = 1 WHERE patientsid = ".$_SESSION['currentptnum'];
                mysqli_query($dbc, $setUQ);
                echo'
                <script>
                window.location = "pt4entry.php";
                </script>';
            }
            else
            {
...
{
    $setUQ = "UPDATE codes AS a INNER JOIN patientusers AS b ON a.patientsid=b.patientsid SET a.day4used = 1 WHERE b.username = '".$_SESSION['username']."'";
    mysqli_query($dbc, $setUQ);
    echo'
    <script>
    window.location = "pt4entry.php";
